...
export const generateRoyaltyPDF = (
  ...
  labelValue('Libro:', statement.bookTitle);
  labelValue(
    'Período:',
  ...
  labelValue('Porcentaje de regalías:', `${statement.royaltyPercentage}%`);

  // --- Tabla de ventas en papel ---
  const body = statement.lines.map((l) => [
    l.invoiceNumber,
    formatNumber(l.quantity),
    formatCurrency(l.pvp),
    fmtDate(l.date),
    formatCurrency(l.totalInvoiced),
    formatCurrency(l.totalRoyalty),
  ]);

  const totalCopies = statement.lines.reduce((s, l) => s + l.quantity, 0);
  ...
  autoTable(doc, {
    ...
    foot: [
      [
        ...
        { content: formatNumber(totalCopies), styles: { halign: 'center' } },
        '',
        '',
        '',
        {
          content: formatCurrency(num(statement.totalRoyalties)),
          styles: { halign: 'right' },
        },
      ],
    ],
