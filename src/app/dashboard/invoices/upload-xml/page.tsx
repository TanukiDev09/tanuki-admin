...
export default function UploadXMLPage() {
  ...
  const processFiles = async () => {
    ...
    for (let i = 0; i < uploadedFiles.length; i++) {
      ...
      try {
        ...
        const res = await fetch('/api/invoices/upload-xml', {
          method: 'POST',
          body: formData,
        });
        if (!res.ok) {
          const errorData = await res
            .json()
            .catch(() => ({ error: 'Error al procesar el archivo' }));
  ...
  const updateFile = (i: number, update: Partial<UploadedFile>) => {
    setUploadedFiles((prev) =>
      prev.map((f, idx) => (idx === i ? { ...f, ...update } : f))
    );
  };
  ...
  const saveAll = async () => {
    ...
    let ok = 0,
      fail = 0;
    for (let i = 0; i < uploadedFiles.length; i++) {
      if (uploadedFiles[i].status !== 'success' || !uploadedFiles[i].invoice)
        continue;
    ...
    reviewingIndex === null && (
      <div className={styles.steps}>
        {[1, 2, 3, 4].map((s) => (
          <div
            key={s}
            className={`${styles.step} ${currentStep >= s ? styles.active : ''}`}
          >
