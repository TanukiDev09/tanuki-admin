...
export default function DebtDetailPage() {
  ...
  if (isLoading) return (
    <div className="flex items-center justify-center min-h-[60vh]">
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="text-slate-500 font-black tracking-widest uppercase text-[10px]">Sincronizando balances...</p>
      </div>
    </div>
  );
  ...
  if (!debt) return (
    <div className="p-20 text-center bg-slate-50 min-h-screen">
      <div className="bg-white p-12 rounded-[2rem] shadow-xl inline-flex flex-col items-center gap-4 max-w-md border border-slate-200">
        ...
        <h2 className="text-2xl font-black text-slate-900 tracking-tight">Registro no encontrado</h2>
        <p className="text-slate-600 font-medium leading-relaxed">Lo sentimos, no pudimos localizar la deuda solicitada.</p>
        <Button
          variant="default"
  ...
  const paidPercentage = totalAmount > 0 ? Math.min(100, Math.max(0, (paidAmount / totalAmount) * 100)) : 0;
      ...
      <div className="debt-detail__header">
        <div className="debt-detail__header-container">
          <div className="debt-detail__header-info">
            ...
            <div>
              <h1 className="debt-detail__header-title">{debt.notes || 'Detalle de Deuda'}</h1>
              <p className="debt-detail__header-subtitle">{debt.type} • ID: {debtId.slice(-6)}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            {isPaid ? (
              ...
              <Badge variant="destructive" className="bg-rose-600 text-white border-none px-4 py-1.5 rounded-full text-[10px] font-black tracking-tight shadow-lg shadow-rose-100">
                VENCIDO
              </Badge>
            ) : (
        ...
        <div className="space-y-8">
          ...
          <div className="debt-detail__hero">
            <div className="relative z-10">
              ...
              <p className="debt-detail__hero-balance-label">Saldo por Liquidar</p>
              ...
              <div className="debt-detail__hero-stats">
                <div className="debt-detail__hero-stat-item">
                  ...
                  <p className="debt-detail__hero-stat-value">{formatCurrency(totalAmount, debt.currency || 'COP')}</p>
                </div>
                <div className="w-[1px] h-10 bg-slate-200 hidden sm:block" />
                <div className="debt-detail__hero-stat-item">
                  <p className="debt-detail__hero-stat-label">Total Pagado</p>
                  <p className="debt-detail__hero-stat-value debt-detail__hero-stat-value--positive">{formatCurrency(paidAmount, debt.currency || 'COP')}</p>
                </div>
              </div>
          ...
          <div className="debt-detail__info-grid">
            <div className="debt-detail__info-card">
              ...
              <div className="debt-detail__info-content">
                <div>
                  <h3 className="debt-detail__info-label">Concepto del Documento</h3>
                  <p className="debt-detail__info-value">{debt.notes || 'Sin descripción'}</p>
                </div>
                <div className="debt-detail__info-subgrid">
                  <div>
                    <p className="debt-detail__info-label">Tipo</p>
                    <p className="text-sm font-black text-slate-900">{debt.type}</p>
                  </div>
                  <div>
                    <p className="debt-detail__info-label">Referencia</p>
                    <p className="text-sm font-black text-slate-900 truncate">{debt.source.reference}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="debt-detail__info-card">
              ...
              <div className="debt-detail__info-content">
                <div>
                  ...
                  <p className="debt-detail__info-value">{new Date(debt.createdAt).toLocaleDateString()}</p>
                </div>
                <div className="debt-detail__info-subgrid">
                  <div>
                    ...
                    <p className={cn(
                      ...
                      {debt.dueDate ? new Date(debt.dueDate).toLocaleDateString() : 'Indefinida'}
                    </p>
                  </div>
                  {isOverdue && (
                    <div className="flex items-end">
                      <Badge variant="destructive" className="h-6 text-[9px] rounded-lg px-2 font-black uppercase">Urgente</Badge>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="debt-detail__timeline-header">
            ...
            <div>
              <h2 className="debt-detail__timeline-title">Historial de Transacciones</h2>
              <p className="debt-detail__timeline-subtitle">Cronología completa de abonos y débitos</p>
            </div>
          </div>
          ...
          {movements.length > 0 ? (
              ...
              {movements.map((mov: any) => (
                <div
                  ...
                  className={cn(
                    ...
                    mov.type === 'Ingreso' ? "debt-detail__timeline-item--income" : "debt-detail__timeline-item--expense"
                  )}
                  ...
                  <div className="debt-detail__movement-card">
                    <div className="debt-detail__movement-main">
                      <div className={cn(
                        ...
                        mov.type === 'Ingreso' ? "debt-detail__movement-icon--income" : "debt-detail__movement-icon--expense"
                      )}>
                        {mov.type === 'Ingreso' ? <ArrowRight className="w-7 h-7" /> : <ArrowLeft className="w-7 h-7" />}
                      </div>
                      <div className="debt-detail__movement-info">
                        <div className="flex items-center gap-3">
                          <h5 className="debt-detail__movement-title">{mov.description || 'Movimiento de pago'}</h5>
                          <Badge className="bg-slate-100 text-slate-800 text-[9px] font-black px-2 py-0.5 rounded-lg border-none">{mov.paymentChannel || 'EFECTIVO'}</Badge>
                        </div>
                    ...
                    <div className="text-right">
                      <p className={cn(
                        ...
                        mov.type === 'Ingreso' ? "debt-detail__movement-amount--income" : "debt-detail__movement-amount--expense"
                      )}>
                        {mov.type === 'Ingreso' ? '+' : '-'}{formatCurrency(mov.amount, mov.currency || 'COP')}
                      </p>
            ...
            <div className="py-24 text-center bg-white rounded-[2rem] border-2 border-dashed border-slate-200 shadow-sm">
              ...
              <h3 className="text-xl font-black text-slate-900">Sin historial de pagos</h3>
              <p className="text-slate-500 font-medium mt-2">Aún no se han detectado abonos vinculados a esta cuenta.</p>
              {!isPaid && (
                <Button
                  variant="default"
                  ...
                  asChild
                >
                  <Link href={`/dashboard/movements/crear?debtId=${debtId}&amount=${remainingBalance}`}>
                    <Zap className="w-4 h-4 mr-2" /> Iniciar Plan de Pago <ArrowRight className="w-4 h-4 ml-2" />
                  </Link>
                </Button>
              )}
            </div>
          ...
          <div className="debt-detail__account-card">
            ...
            <div className="debt-detail__sidebar-actions">
              <Button
                ...
                asChild
              >
                <Link href={`/dashboard/movements/crear?debtId=${debtId}&amount=${remainingBalance}&type=${debt.type === 'Cuenta por Cobrar' ? 'Ingreso' : 'Egreso'}`}>
                  <DollarSign className="w-5 h-5 mr-2" /> Registrar Pago <ArrowRight className="w-5 h-5 ml-2" />
                </Link>
              </Button>
          ...
          <div className="bg-white rounded-[2rem] p-8 border border-slate-200/60 shadow-sm space-y-4">
            ...
            <div className="space-y-3">
              {debt.source.type === 'Invoice' && (
                <Button variant="outline" className="w-full justify-between h-14 rounded-2xl hover:bg-slate-50 border-slate-200 px-6 font-black text-slate-700" asChild>
              ...
              <Button variant="outline" className="w-full justify-between h-14 rounded-2xl hover:bg-slate-50 border-slate-200 px-6 font-black text-slate-700">
                <div className="flex items-center gap-3">
            ...
            <DialogHeader>
              <div className="flex items-center gap-4 text-rose-600 mb-2">
                ...
                <div className="text-left">
                  <DialogTitle className="text-2xl font-black tracking-tight">Confirmar Eliminación</DialogTitle>
                  <p className="text-[10px] font-black text-rose-400 uppercase tracking-widest">Acción Irreversible</p>
                </div>
              </div>
              <DialogDescription className="text-slate-600 font-medium text-base leading-relaxed pt-4">
                ¿Estás completamente seguro de que deseas eliminar esta deuda? Esta operación retirará la obligación de los balances consolidados.
                {paidAmount > 0 && (
